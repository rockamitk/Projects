...
public class Bill_Payment extends JFrame 
{
    ...
    int c,k;           
     ...
     String[] fieldhead={"Invoice No","Customer Code","BILL Amount","PAID Amount","DUES Amount"};
     Object cord[][]=new Object[20][5];
       ...
       public Bill_Payment()
        
        {
              ...
              Container cn=fr.getContentPane();
                ...
                cn.setLayout(null);
                icn=new ImageIcon("f.jpg");
                l=new JLabel(icn);
                l.setBounds(0,0,278,800);
 ...
 public void invoiceComboBox()
        {
          try
             {
               cb1.addItem("     -SELECT-");
               rs=statement.executeQuery("select invno from Issue_Bill group by invno order by invno");
               while(rs.next())
               {
   ...
   public void billAmountFind(String st)
	...
	try

            {
             ...
             rs=statement.executeQuery("select billamt from Issue_Bill where invno='"+st+"'");
   ...
   public void duesAmountFind(String s1)
	...
	try

            {
             ...
             rs=statement.executeQuery("select osamt from Bulk_Customer where ccode='"+s1+"'");
            ...
            try
              {
                ...
                rs=statement.executeQuery("select * from Bill_Payment where invno='"+str1+"'");
               while(rs.next())
                  {
                ...
                if(flg==1)
                  {
                   JOptionPane.showMessageDialog(null,"Bill Paid For "+str1+" Invoice ");
                  }
                else
                  { 
                   str1=(String)cb1.getSelectedItem();
                   ...
                   statement.executeUpdate("insert into Bill_Payment values('"+str1+"','"+str2+"','"+str3+"','"+str4+"','"+str5+"')");
                   statement.executeUpdate("commit");
                   statement.executeUpdate("Update Bulk_Customer set osamt='"+str5+"' where ccode='"+str2+"'");
                   statement.executeUpdate("commit");
                   load();
     ...
     public void actionPerformed(ActionEvent e)
       {
        try
           {
            str=JOptionPane.showInputDialog("Enter Bill_Payment to be delete");
            statement.executeUpdate("delete from Bill_Payment where invno='"+str+"'");
            statement.executeUpdate("Commit");
            load();
        ...
        try
           {
            ...
            statement.executeUpdate("Update Bill_Payment set ccode='"+str2+"', billamt='"+str3+"', paidamt='"+str4+"', osamt='"+str5+"' where invno='"+str1+"'");
            statement.executeUpdate("commit");
            statement.executeUpdate("Update Bulk_Customer set osamt='"+str5+"' where ccode='"+str2+"'");
            statement.executeUpdate("commit");
            load();
      ...
      public void actionPerformed(ActionEvent e)
        {
         try
            {
             s1=JOptionPane.showInputDialog("Enter the Invoice No to be search");
    ...
    public void load()
      {
       int k=0;
        ...
        try
         {
          statement.executeUpdate("commit");
          rs=statement.executeQuery("select * from Bill_Payment order by invno");
          while(rs.next())
           {
            str1=rs.getString(1);
            str2=rs.getString(2);
