...
public class searchParty_Invoice extends JFrame 
{

  private JLabel l1,l2,l3;
  
  private JTextField t3,t2;
  ...
  //  jsp.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_NEVER);
   ...
   public void invoiceCombo()
        ...
        private class cbListener implements ActionListener
        {
			...
			try
			{
                        rs=statement.executeQuery("select distinct(pcode) from Party_Invoice where invno='"+String.valueOf(cb1.getSelectedItem()).trim()+"'");
                        rs.next();
			str=rs.getString(1);
			t2.setText(str);
			}
      ...
      { try
          {
             ...
             load(str6,str7);
             rs=statement.executeQuery("select invno,sum(invamt) from Party_Invoice group by invno having invno='"+str6+"'");
        ...
        try
         {
          ...
          rs=statement.executeQuery("select distinct(pdcode),TO_CHAR(invdate,'DD-MON-YYYY'),qty,mrp,invamt from Party_Invoice where invno='"+str6+"' and pcode='"+str7+"'order by pdcode");
...
/*
  ...
  {
        searchParty_Invoice spi= new searchParty_Invoice();
  }*/
}
