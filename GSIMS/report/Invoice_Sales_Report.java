...
public class Invoice_Sales_Report extends JFrame 
{
 ...
 public Invoice_Sales_Report()
 {
       ...
       setLayout(null);
        ...
        cn=getContentPane();
        cn.setBackground(new Color(180,180,100));

        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);

        tl=new JLabel("Invoice sales Report");
 ...
 public void month()
        {
          try
             {
               cb1.addItem(" -SELECT-");
               rs=statement.executeQuery("select distinct(month) from bCustomer_Invoice order by month");
               while(rs.next())
               {
 ...
 public void year()
        {
          try
             {
               cb2.addItem(" -SELECT-");
               rs=statement.executeQuery("select distinct(year) from bCustomer_Invoice order by year");
               while(rs.next())
               {
     ...
     try
       {
          ...
          str2=s.substring(2);
          rs=statement.executeQuery("select distinct(pdcode) from bCustomer_Invoice where month='"+str1+"' and year='"+str2+"' order by pdcode");
          while(rs.next())
          {
           str3=rs.getString(1);


           rs1=statement1.executeQuery("select sum(qty) from bCustomer_Invoice where pdcode='"+str3.trim()+"'");
           rs1.next();
     ...
     try
       {
          ...
          s=(String)cb2.getSelectedItem();
          str2=s.substring(2);
          rs=statement.executeQuery("select distinct(invno) from bCustomer_Invoice where month='"+str1+"' and year='"+str2+"' order by invno");
          while(rs.next())
          {
           st=rs.getString(1);
            rs1=statement1.executeQuery("select billamt from Issue_Bill where invno='"+st.trim()+"'");
            rs1.next();
            str=rs1.getString(1);
...
private class setupListener implements ActionListener
{
 ...
 {
      Invoice_Sales_Report msr=new Invoice_Sales_Report();
 }
 */ 
}
